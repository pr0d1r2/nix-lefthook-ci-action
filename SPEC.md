...
| B8 | 2026-09-25 | unit tests loaded undeclared Bats libraries and invoked the intentionally non-executable wrapper directly, so setup or every command test failed in the guardrails environment | use built-in Bats assertions and invoke the wrapper explicitly with `bash`, matching the action |
...
| B11 | 2026-08-13 | guardrails `fidelity` check remained red because the tracked `lefthook.yml` was materialized without the `actions` fragment required by the workflow-based fragment detector | regenerate `lefthook.yml` with the detected `actions` fragment |
| B12 | 2026-08-13 | guardrails coherence check failed because the materialized `actions` fragment referenced `lefthook-actionlint`, but the consumer flake exposed only an actionlint check and not the wrapper package in its dev shell | expose the `lefthook-actionlint` wrapper through `mkConsumerFlake`'s `extraPackages` |
| B13 | 2026-08-13 | guardrails fidelity kept detecting the `actions` fragment from `.github/workflows/ci.yml`, but the consumer flake did not declare that fragment, so generated `lefthook.yml` could not match the required materialization | declare the `actions` fragment in `flake.nix` and regenerate `lefthook.yml` |
| B14 | 2026-08-13 | the pinned `set-and-setting` `mkActionlintCheck` passed a scalar workflow path regex to Nixpkgs `sourceByRegex`, whose current API requires a list | override the helper in the consumer flake with an equivalent extension-filtered actionlint check that avoids the incompatible path-prefix call |
| B15 | 2026-08-14 | the flake-manifest guardrail structurally scanned the delegated `outputs` expression and interpreted the nested `materializationFor = args: let` helper as an illegal outputs-level let expression | parenthesize the nested let expression so the manifest parser can distinguish it from the outputs delegation |
| B16 | 2026-09-25 | guardrails detected Bats tests but the consumer flake omitted the `bats` fragment, and the materialized `lefthook.yml` was ignored by Git, so fidelity could never match the current fragment set | declare the `bats` fragment, regenerate `lefthook.yml`, and track the materialized configuration |

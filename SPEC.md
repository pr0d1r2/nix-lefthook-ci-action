...
| B16 | 2026-09-25 | guardrails exited 127 in the actionlint hook because the `lefthook-actionlint` wrapper was exposed without its `actionlint` executable on the dev-shell `PATH` | expose `actionlint` alongside the wrapper package; track the materialized `lefthook.yml` instead of ignoring it |

...
| B16 | 2026-09-11 | guardrails actionlint hook exited 127 because the generated wrapper could not resolve `actionlint` from `PATH` despite declaring it as a runtime input | invoke the Nix-packaged actionlint binary by its absolute store path in both local wrapper definitions |
